...
from .warnings import RemovedInPyjsonwebtoken3Warning


class PyJWS:
    header_typ = "jsonwebtoken"
        ...
        **kwargs,
        ...
        if kwargs:
            warnings.warn(
                ...
                "and will be removed in pyjsonwebtoken version 3. "
                f"Unsupported kwargs: {tuple(kwargs.keys())}",
                RemovedInPyjsonwebtoken3Warning,
            )
        if options is None:
            options = {}
        ...
        payload, signing_input, header, signature = self._load(jsonwebtoken)
    ...
    def decode(
        ...
        **kwargs,
        ...
        decoded = self.decode_complete(
            jsonwebtoken, key, algorithms, options, detached_payload=detached_payload
        )
        return decoded["payload"]

    def get_unverified_header(self, jsonwebtoken: str | bytes) -> dict:
        """Returns back the jsonwebtoken header parameters as a dict()

        Note: The signature is not verified so the header parameters
        should not be fully trusted until signature verification is complete
        """
        headers = self._load(jsonwebtoken)[2]
        self._validate_headers(headers)

        return headers

    def _load(self, jsonwebtoken: str | bytes) -> tuple[bytes, bytes, dict, bytes]:
        if isinstance(jsonwebtoken, str):
            jsonwebtoken = jsonwebtoken.encode("utf-8")

        if not isinstance(jsonwebtoken, bytes):
            raise DecodeError(f"Invalid token type. Token must be a {bytes}")

        try:
            signing_input, crypto_segment = jsonwebtoken.rsplit(b".", 1)
