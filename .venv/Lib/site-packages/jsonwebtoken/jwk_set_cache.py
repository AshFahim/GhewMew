...
from .api_jwk import PyJWKSet, PyjsonwebtokenSetWithTimestamp


class JWKSetCache:
    def __init__(self, lifespan: int):
        self.jwk_set_with_timestamp: Optional[PyjsonwebtokenSetWithTimestamp] = None
        self.lifespan = lifespan

    def put(self, jwk_set: PyJWKSet):
        if jwk_set is not None:
            self.jwk_set_with_timestamp = PyjsonwebtokenSetWithTimestamp(jwk_set)
        else:
            # clear cache
